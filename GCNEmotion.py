# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 14:43:06 2024
...
def interpolate_motion_data(joint_motion_data, num_frames, max_frames):
    ...
    for joint_name, channel_data in joint_motion_data.items():
        # Ensure channel_data is a NumPy array for easy manipulation
...
processed_data_path = 'Emotions Four Main/processed/data.pt'   
...
# category_names = ['Angry', 'Depressed', 'Neutral', 'Proud'] 
...
# plt.legend(title='Emotion', title_fontsize='15', fontsize='14')  
# plt.grid(True)
# plt.show()
...
# Define class index to name mapping for the emotional states
class_names = {0: 'Angry', 1: 'Depressed', 2: 'Neutral', 3: 'Proud'}
...
def test(model, loader, print_metrics=False):
    ...
    if print_metrics:
        # Replace numeric labels with emotional state names using the mapping
...
def main():
    ...
    # DataLoader for both training and testing datasets
    ...
    plt.rcParams.update({'font.size': 14, 'font.weight': 'bold', 'axes.labelweight': 'bold', 
                         'axes.titleweight': 'bold', 'lines.linewidth': 2})
...
# from sklearn.manifold import TSNE
# # Data generation
# node_features_after = [np.random.rand(155, 4) for _ in range(9)]
...
# labels_per_category = total_samples // 4  # Evenly distribute labels
# remainder = total_samples % 4  # Calculate any leftover samples
# # Creating labels with possible adjustments for remainder
# labels = []
# categories = [0, 1, 2, 3]  # Representing Angry, Depressed, Neutral, Proud
# for i, category in enumerate(categories):
...
# # Run t-SNE
# tsne = TSNE(n_components=2, perplexity=2, learning_rate=200, n_iter=500, random_state=42)
